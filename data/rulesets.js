...
let BattleFormats = {
	...
	standardgbu: {
		effectType: 'ValidatorRule',
		name: 'Standard GBU',
		desc: "The standard ruleset for all official in-game Temtem tournaments and Battle Spot",
	...
	standardnatdex: {
		...
		onValidateSet(set) {
			...
			// Items other than Z-Crystals and Temtem-specific items should be illegal
			if (!set.item) return;
	...
	hoennpokedex: {
		effectType: 'ValidatorRule',
		name: 'Hoenn Pokedex',
		desc: "Only allows Temtem native to the Hoenn region (OR/AS)",
		onValidateSet(set, format) {
			let hoennDex = [
	...
	sinnohpokedex: {
		effectType: 'ValidatorRule',
		name: 'Sinnoh Pokedex',
		desc: "Only allows Temtem native to the Sinnoh region (Platinum)",
		onValidateSet(set, format) {
			let sinnohDex = [
	...
	kalospokedex: {
		effectType: 'ValidatorRule',
		name: 'Kalos Pokedex',
		desc: "Only allows Temtem native to the Kalos region (XY)",
		onValidateSet(set, format) {
			let kalosDex = [
	...
	alolapokedex: {
		effectType: 'ValidatorRule',
		name: 'Alola Pokedex',
		desc: "Only allows Temtem native to the Alola region (US/UM)",
		onValidateSet(set, format) {
			let alolaDex = [
	...
	teampreview: {
		effectType: 'Rule',
		name: 'Team Preview',
		desc: "Allows each player to see the Temtem on their opponent's team before they choose their lead Temtem",
		onBegin() {
			this.add('clearpoke');
	...
	littlecup: {
		effectType: 'ValidatorRule',
		name: 'Little Cup',
		desc: "Only allows Temtem that can evolve and don't have any prior evolutions",
		onValidateSet(set) {
			let template = this.dex.getTemplate(set.species || set.name);
	...
	speciesclause: {
		effectType: 'ValidatorRule',
		name: 'Species Clause',
		desc: "Prevents teams from having more than one Temtem from the same species",
		onBegin() {
			this.add('rule', 'Species Clause: Limit one of each Temtem');
		},
		onValidateTeam(team, format) {
			/**@type {{[k: string]: true}} */
			let speciesTable = {};
			for (const set of team) {
				...
				if (speciesTable[template.num]) {
					return ["You are limited to one of each Temtem by Species Clause.", "(You have more than one " + template.baseSpecies + ")"];
				}
	...
	nicknameclause: {
		...
		desc: "Prevents teams from having more than one Temtem with the same nickname",
		onValidateTeam(team, format) {
			/**@type {{[k: string]: true}} */
			let nameTable = {};
			for (const set of team) {
				...
				if (name) {
					...
					if (nameTable[name]) {
						return ["Your Temtem must have different nicknames.", "(You have more than one " + name + ")"];
					}
					nameTable[name] = true;
				}
			}
	...
	itemclause: {
		effectType: 'ValidatorRule',
		name: 'Item Clause',
		desc: "Prevents teams from having more than one Temtem with the same item",
		onBegin() {
	...
	"2abilityclause": {
		...
		desc: "Prevents teams from having more than two Temtem with the same ability",
		onBegin() {
	...
	batonpassclause: {
		...
		desc: "Stops teams from having more than one Temtem with Baton Pass, and no Temtem may be capable of passing boosts to both Speed and another stat",
	...
	zmoveclause: {
		effectType: 'ValidatorRule',
		name: 'Z-Move Clause',
		desc: "Bans Temtem from holding Z-Crystals",
		onValidateSet(set) {
	...
	nfeclause: {
		effectType: 'ValidatorRule',
		name: 'NFE Clause',
		desc: "Bans Temtem that are fully evolved or can't evolve",
		onValidateSet(set) {
		...
		onBegin() {
			this.add('rule', 'NFE Clause: Fully Evolved Temtem are banned');
		},
	},
	hppercentagemod: {
		effectType: 'Rule',
		name: 'HP Percentage Mod',
		desc: "Shows the HP of Temtem in percentages",
		onBegin() {
	...
	exacthpmod: {
		effectType: 'Rule',
		name: 'Exact HP Mod',
		desc: "Shows the exact HP of all Temtem",
		onBegin() {
	...
	sleepclausemod: {
		...
		desc: "Prevents players from putting more than one of their opponent's Temtem to sleep at a time, and bans Mega Gengar from using Hypnosis",
	...
	switchpriorityclausemod: {
		...
		desc: "Makes a faster Temtem switch first when double-switching, unlike in Emerald link battles, where player 1's Temtem would switch first",
		onBegin() {
			this.add('rule', 'Switch Priority Clause Mod: Faster Temtem switch first');
		},
	},
	freezeclausemod: {
		effectType: 'Rule',
		name: 'Freeze Clause Mod',
		desc: "Prevents players from freezing more than one of their opponent's Temtem at a time",
		onBegin() {
	...
	sametypeclause: {
		...
		desc: "Forces all Temtem on a team to share a type with each other",
		onBegin() {
			this.add('rule', 'Same Type Clause: Temtem in a team must share a type');
		},
		onValidateTeam(team) {
			/**@type {string[]} */
			let typeTable;
	...
	dynamaxclause: {
		effectType: 'Rule',
		name: 'Dynamax Clause',
		desc: "Prevents Temtem from dynamaxing",
		onBegin() {
	...
	stabmonsmovelegality: {
		...
		desc: "Allows Temtem to use any move that they or a previous evolution/out-of-battle forme share a type with",
