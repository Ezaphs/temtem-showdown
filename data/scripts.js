...
let BattleScripts = {
	...
	runMove(moveOrMoveName, pokemon, targetLoc, sourceEffect, zMove, externalMove, maxMove, originalTarget) {
		...
		let staMod = 1;
		if (pokemon.volatiles['vigorized']) staMod -= 0.5;
		if (pokemon.volatiles['exhausted']) staMod += 0.5;
		pokemon.sta -= Math.floor(move.sp * staMod);
		if (pokemon.sta < 0) {
			this.damage(pokemon.sta*-2, pokemon, pokemon, '[silent]');
			pokemon.addVolatile('mustrecharge');
			pokemon.sta = 0;
			this.add('message', pokemon.name + ` overexerted itself and took damage!`);
		}
		else this.add('html', pokemon.name + `'s Stamina: ` + Math.floor(pokemon.sta*100/pokemon.maxsta) + `%`);
