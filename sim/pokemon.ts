...
export class Pokemon {
	...
	constructor(set: string | AnyObject, side: Side) {
		...
		if (!this.set.evs) {
			this.set.evs = {hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0, sta: 0};
		}
		if (!this.set.ivs) {
			this.set.ivs = {hp: 50, atk: 50, def: 50, spa: 50, spd: 50, spe: 50, sta: 50};
		}
		const stats: StatsTable = {hp: 50, atk: 50, def: 50, spe: 50, spa: 50, spd: 50, sta: 50};
		let stat: StatName;
		for (stat in stats) {
		...
		for (stat in this.set.evs) {
			this.set.evs[stat] = this.battle.dex.clampIntRange(this.set.evs[stat], 0, 500);
		}
		for (stat in this.set.ivs) {
			this.set.ivs[stat] = this.battle.dex.clampIntRange(this.set.ivs[stat], 0, 50);
		}
		...
		this.permaboosts = {atk: 0, def: 0, spa: 0, spd: 0, spe: 0, accuracy: 0, evasion: 0};
	...
	*/
	...
	ignoringItem() {
		return !!((this.battle.gen >= 5 && !this.isActive) ||
			...
			this.volatiles['seized'] || this.battle.field.pseudoWeather['magicroom']);
	}
	...
	addHold(move: string | Move, target?: Pokemon | null | false) {
		move = this.battle.dex.getMove(move) as Move;
		const ppData = this.getMoveData(move);
		if (!ppData) return 0;
		ppData.used = true;
	...
	{move: string, id: string, disabled?: string | boolean,
		...
		for (const moveSlot of this.moveSlots) {
			...
			if (this.volatiles['dynamax']) {
				...
				(moveSlot.pp < moveSlot.maxpp && !this.volatiles['partialtrappinglock']) || disabled &&
	...
	clearVolatile(includeSwitchFlags = true) {
		...
		this.permaboosts = this.boosts;
		this.permavolatiles = this.volatiles;
		this.transformed = false;
		this.ability = this.baseAbility;
	...
	/** Returns the amount of stamina actually healed */
	healsta(d: number, source: Pokemon | null = null, effect: Effect | null = null) {
		d = this.battle.trunc(d);
		if (isNaN(d)) return false;
		if (d <= 0) return false;
		if (this.sta >= this.maxsta) return false;
		this.sta += d;
		if (this.sta > this.maxsta) {
			d -= this.sta - this.maxsta;
			this.sta = this.maxsta;
		}
		this.battle.add('message', this.name + ` regained ` + Math.floor(d*100/this.maxsta) + `% Stamina- now at ` + Math.floor(this.sta*100/this.maxsta) + `%.`);
		return d;
	}
	...
	trySetStatus(status: string | PureEffect, source: Pokemon | null = null, sourceEffect: Effect | null = null, duration: number) {
	...
	setStatus(
		...
		this.statusData.durval = duration;
	...
	addVolatile(
		...
		let oldest = "";
		let oldage = 0;
		for (const vola in this.volatiles) {
			if(vola != "resting" && vola != "mustrecharge") {
				if (this.volatiles[vola].age == "OLD") this.removeVolatile(vola);
				if (!this.volatiles[vola].age) this.volatiles[vola].age = 0;
				this.volatiles[vola].age++;
				if (this.volatiles[vola].age > oldage) {
					oldest = vola;
					oldage = this.volatiles[vola].age;
				}
				volCount++;
			}
		}
		if (volCount > 1) {
			this.removeVolatile(oldest);
			this.battle.add('-end', this, oldest);
		}
