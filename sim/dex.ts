...
export class ModdedDex {
	...
	spreadModify(baseStats: StatsTable, set: PokemonSet): StatsTable {
		const modStats: SparseStatsTable = {atk: 10, def: 10, spa: 10, spd: 10, spe: 10, sta: 10};
		const uniqueMods = {hp: [80,20000,15], atk: [100,25000,10], def: [100,25000,10], spa: [100,25000,10], spd: [100,25000,10], spe: [100,25000,10], sta: [200,25000,20]};
		const tr = this.trunc;
		let statName: keyof StatsTable;
		for (statName in modStats) {
			const stat = baseStats[statName];
			const svs = 50;
			const tvs = set.evs[statName];
			const cons = uniqueMods[statName];
			if (svs == 0) svs = 1;
			let val1 = set.level * ((1.5*stat) + svs + (tvs/5)) / cons[0];
			let val2 = (svs * stat * set.level / cons[1]);
			modStats[statName] = tr(val1 + val2 + cons[2]);
		}
		if ('hp' in baseStats) {
			const stat = baseStats['hp'];
			const svs = 50;
			const tvs = set.evs['hp'];
			const cons = uniqueMods['hp'];
			let val1 = set.level * ((1.5*stat) + svs + (tvs/5)) / cons[0];
			let val2 = (svs * stat * set.level / cons[1]);
			modStats['hp'] = tr(val1 + val2 + cons[2]) + set.level;
		}
		return modStats;
	}
	...
	packTeam(team: PokemonSet[] | null): string {
		...
		for (const set of team) {
			...
			let evs = '|';
			if (set.evs) {
				evs = '|' + (set.evs['hp'] || '') + ',' + (set.evs['atk'] || '') + ',' + (set.evs['def'] || '') + ',' + (set.evs['spa'] || '') + ',' + (set.evs['spd'] || '') + ',' + (set.evs['spe'] || '') + (set.evs['sta'] || '');
			}
			if (evs === '|,,,,,') {
				buf += '|';
			} else {
			...
			if (set.ivs) {
				ivs = '|' + getIv(set.ivs, 'hp') + ',' + getIv(set.ivs, 'atk') + ',' + getIv(set.ivs, 'def') +
					',' + getIv(set.ivs, 'spa') + ',' + getIv(set.ivs, 'spd') + ',' + getIv(set.ivs, 'spe') + getIv(set.ivs, 'sta');
			}
			if (ivs === '|,,,,,') {
				buf += '|';
			} else {
	...
	fastUnpackTeam(buf: string): PokemonSet[] | null {
		...
		for (let count = 0; count < 24; count++) {
			...
			if (j < 0) return null;
			if (j !== i) {
				const evs = buf.substring(i, j).split(',', 7);
				set.evs = {
					hp: Number(evs[0]) || 0,
					atk: Number(evs[1]) || 0,
					def: Number(evs[2]) || 0,
			...
			if (j < 0) return null;
			if (j !== i) {
				const ivs = buf.substring(i, j).split(',', 7);
				set.ivs = {
					hp: ivs[0] === '' ? 50 : Number(ivs[0]) || 0,
					atk: ivs[1] === '' ? 50 : Number(ivs[1]) || 0,
					def: ivs[2] === '' ? 50 : Number(ivs[2]) || 0,
					spa: ivs[3] === '' ? 50 : Number(ivs[3]) || 0,
					spd: ivs[4] === '' ? 50: Number(ivs[4]) || 0,
					spe: ivs[5] === '' ? 50 : Number(ivs[5]) || 0,
					sta: ivs[6] === '' ? 50 : Number(ivs[6]) || 0,
				};
			}
			i = j + 1;
