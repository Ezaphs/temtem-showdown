...
export class TeamValidator {
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (!set.evs) set.evs = TeamValidator.fillStats(null, allowEVs && !capEVs ? 500 : 0);
		if (!set.ivs) set.ivs = TeamValidator.fillStats(null, 50);
		...
		if (set.moves.includes('Rest')) {
			problems.push(`Remove Rest from ${name}- it will be added automatically.`);
		}
		else set.moves.push('rest');
		if (ruleTable.isBanned('nonexistent') && set.moves.length > 5) {
	...
	validateStats(set: PokemonSet, template: Template, setSources: PokemonSources) {
		...
		const statTable = {
			hp: 'HP', atk: 'Attack', def: 'Defense', spa: 'Special Attack', spd: 'Special Defense', spe: 'Speed', sta: 'Stamina'
		};
		...
		if (set.hpType && maxedIVs && ruleTable.has('obtainablemisc')) {
			if (dex.gen <= 2) {
				...
				set.ivs = {hp: 30, atk: 30, def: 30, spa: 30, spd: 30, spe: 30, sta: 30};
		...
		if (dex.currentMod === 'letsgo') { // AVs
			...
			for (const stat in set.evs) {
				if (set.evs[stat as 'hp'] > 500) {
					problems.push(`${name} has more than 500 TVs in ${statTable[stat as 'hp']}.`);
				}
			}
		}
		...
		if (set.evs['sta'] == 0) set.evs['sta'] = Math.min(1000 - totalEV, 500);

		if (!this.format.debug) {
			if ((allowEVs || allowAVs) && totalEV === 0) {
				problems.push(`${name} has exactly 0 TVs - did you forget to TV it? (If this was intentional, add exactly 1 to one of your TVs, which won't change its stats but will tell us that it wasn't a mistake).`);
			} else if (allowEVs && !capEVs && [1000].includes(totalEV)) {
				problems.push(`${name} has exactly 1000 EVs, but this format does not restrict you to 510 EVs: you can max out every EV (If this was intentional, add exactly 1 to one of your EVs, which won't change its stats but will tell us that it wasn't a mistake).`);
			}
			...
			if (set.level === 50 && this.format.maxLevel !== 50 && allowEVs && totalEV % 4 === 0) {
				problems.push(`${name} is level 50, but this format allows level 100 Temtem. (If this was intentional, add exactly 1 to one of your TVs, which won't change its stats but will tell us that it wasn't a mistake).`);
			}
		}

		if (allowEVs && capEVs && totalEV > 1000) {
			problems.push(`${name} has more than 1000 total TVs.`);
		}
	...
	validateEvent(set: PokemonSet, eventData: EventInfo, eventTemplate: Template, because = ``, from = `from an event`) {
		...
		if (eventData.ivs) {
			...
			if (!set.ivs) set.ivs = {hp: 50, atk: 50, def: 50, spa: 50, spd: 50, spe: 50, sta: 50};
			const statTable = {
				hp: 'HP', atk: 'Attack', def: 'Defense', spa: 'Special Attack', spd: 'Special Defense', spe: 'Speed', sta: 'Stamina'
			};
			let statName: StatName;
	...
	checkLearnset(
		...
		if (!moveSources.size()) {
			...
			if (move.name != "Rest") return {type: 'invalid'};
		}
		setSources.intersectWith(moveSources);
		if (!setSources.size() && move.name != "Rest") {
	...
	static fillStats(stats: SparseStatsTable | null, fillNum: number = 0): StatsTable {
		const filledStats: StatsTable = {hp: fillNum, atk: fillNum, def: fillNum, spa: fillNum, spd: fillNum, spe: fillNum, sta: fillNum};
		if (stats) {
			let statName: StatName;
