...
export class Battle {
	...
	comparePriority(a: AnyObject, b: AnyObject) {
		const prioList = [0, 0.5, 1, 1.5, 1.75];
		if(b.priority >= 0 && a.priority >= 0 && b.priority <= 4 && a.priority <= 4) {
			return -((b.order || 4294967296) - (a.order || 4294967296)) ||
			((b.speed * prioList[b.priority] || 0) - (a.speed * prioList[a.priority] || 0)) ||
			((b.speed || 0) - (a.speed || 0)) ||
			-((b.subOrder || 0) - (a.subOrder || 0)) ||
			((a.thing && b.thing) ? -(b.thing.abilityOrder - a.thing.abilityOrder) : 0) ||
			0;
		}
		else return -((b.order || 4294967296) - (a.order || 4294967296)) ||
	...
	static compareRedirectOrder(a: AnyObject, b: AnyObject) {
		const prioList = [0, 0.5, 1, 1.5, 1.75];
		this.add('message', b.priority + " " + a.priority);
	...
	nextTurn() {
		...
		for (const side of this.sides) {
			...
			for (const pokemon of side.active) {
				if (!pokemon) continue;
				//if (pokemon.movesLastUsed.length) {
				//	
				//}
					
				//for (var move of pokemon.movesLastUsed) {
				//	pokemon.movesLastUsed[move]++;
				//}
				...
				let tempsta = 0;
				if (pokemon.volatiles['resting'] && pokemon.sta != pokemon.maxsta) {
					tempsta = Math.min(pokemon.maxsta, pokemon.sta + pokemon.maxsta/5);
				}
				else tempsta = Math.min(pokemon.maxsta, pokemon.sta + pokemon.maxsta/20);
				if (tempsta != pokemon.sta) {
					this.add('message', pokemon.name + ` regained ` + Math.floor((tempsta-pokemon.sta)*100/pokemon.maxsta) + `% Stamina- now at ` + Math.floor(tempsta*100/pokemon.maxsta) + `%.`);
					pokemon.sta = tempsta;
				}
	...
	getDamage(
		...
		let baseDamage = (7 + ((basePower * level * (attack/defense))/200))

		// Calculate damage modifiers separately (order differs between generations)
		baseDamage = this.modifyDamage(baseDamage, pokemon, target, move, suppressMessages);
		return tr(baseDamage);
	}
	...
	modifyDamage(
		...
		if (pokemon.status === 'brn' && !pokemon.hasAbility('guts')) {
			baseDamage = this.modify(baseDamage, 0.7);
		}
