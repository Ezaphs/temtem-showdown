...
export const Chat = new class {
	...
	getDataPokemonHTML(template: Template, gen = 7, tier = '') {
		...
		buf += `<span class="col iconcol"><img src="https://cdn.tem.team/images/temtem/regular/${template.num}.png" alt="${template.num}" height="25" width="25"></span> `;
		buf += `<span class="col pokemonnamecol" style="white-space:nowrap"><a href="https://temtem.gamepedia.com/${template.species}" target="_blank">${template.species}</a></span> `;
		buf += '<span class="col typecol">';
		if (template.types) {
			for (const type of template.types) {
				//buf += `<img src="https://temteam.sfo2.digitaloceanspaces.com/images/types/${type}.png" alt="${type}" height="20" width="20">`;
				buf += `<img src="https://gamewith-en.akamaized.net/article_tools/temtem/gacha/${type}.png" alt="${type}" height="25" width="25">`;
			}
		}
		buf += '</span> ';
		if (gen >= 3) {
		...
		buf += '<span class="col statcol"><em>Sta</em><br />' + template.baseStats.sta + '</span> ';
	...
	getDataMoveHTML(move: Move) {
		...
		buf += `<span class="col movenamecol"><a href="https://temtem.gamepedia.com/${move.name.split(' ').join('_')}">${move.name}</a></span> `;
		// encoding is important for the ??? type icon
		const encodedMoveType = encodeURIComponent(move.type);
		//buf += `<span class="col typecol"><img src="https://temteam.sfo2.digitaloceanspaces.com/images/types/${encodedMoveType}.png" alt="${move.type}" width="32" height="32" title="${move.type}">`;
		buf += `<span class="col typecol"><img src="https://gamewith-en.akamaized.net/article_tools/temtem/gacha/${encodedMoveType}.png" alt="${move.type}" width="25" height="25" title="${move.type}">`;
		buf += `<img src="https://cdn.tem.team/images/techniques/class/${move.category.toLowerCase()}.png" alt="${move.category}" width="24" height="22" title="${move.category}"></span> `;
		// tslint:disable-next-line: max-line-length
		if (move.basePower) buf += `<span class="col labelcol"><em>Power</em><br>${typeof move.basePower === 'number' ? move.basePower : '—'}</span> `;
		buf += `<span class="col widelabelcol"><em>Stamina</em><br>${move.sp || 0}</span> `;
		const pp = move.pp || 1;
		buf += `<span class="col pplabelcol"><em>Hold</em><br>${pp-1}</span> `;
	...
	getDataAbilityHTML(ability: Ability) {
		...
		let buf = `<ul class="utilichart"><li class="result">`;
		buf += `<span class="col namecol"><a href="https://temtem.gamepedia.com/${ability.name.split(' ').join('_')}">${ability.name}</a></span> `;
