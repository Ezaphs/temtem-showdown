	...
	data(target, room, user, connection, cmd) {
		...
		for (const [i, newTarget] of newTargets.entries()) {
			...
			case 'move':
				...
				if (showDetails) {
					const prioList = [" (last)", " (0.5x)", " (1x)", " (1.5x)", " (1.75x)", " (before switch)"];
					details = {
						"Prio": move.priority + prioList[move.priority],
					};
					...
					details["Target"] = {
						'normal': "One Adjacent Temtem",
						'self': "User",
						'adjacentAlly': "One Ally",
						'adjacentAllyOrSelf': "User or Ally",
						'adjacentFoe': "One Adjacent Opposing Temtem",
						'allAdjacentFoes': "All Adjacent Opponents",
						'foeSide': "Opposing Side",
						'allySide': "User's Side",
						'allyTeam': "User's Side",
						'allAdjacent': "All Adjacent Temtem",
						'any': "Any Temtem",
						'all': "All Temtem",
					}[move.target] || "Unknown";
